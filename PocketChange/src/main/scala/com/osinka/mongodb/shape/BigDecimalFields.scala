...
trait BigDecimalFields[T, QueryType] extends ShapeFields[T, QueryType] {
  val rounding = BigDecimal.RoundingMode.FLOOR

  trait BigDecimalContent extends ScalarContent[BigDecimal] { self: MongoField[BigDecimal] =>
    def scale: Int
    private lazy val mult = List.fill(scale)(10) reduceLeft {_ * _}
  ...
  class BigDecimalField(override val mongoFieldName: String, override val scale: Int, val g: T => BigDecimal, val p: Option[(T,BigDecimal) => Unit]) extends MongoScalar[BigDecimal] with BigDecimalContent with ScalarFieldModifyOp[BigDecimal] {
    ...
    def fromString(value: String) = BigDecimal(value).setScale(scale, rounding)
  }
}
